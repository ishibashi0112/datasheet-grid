...
- set / select フィルターの**候補はクライアントが供給**します(全行を持たないため自動収集できません)。低カーディナリティ列は `filterOptions` を静的指定、サーバから取りたい列はグリッド prop `getFilterOptions`(popover を開くたびに非同期で候補を返す。[ソートとフィルター](/docs/guides/filter-sort) を参照)、高カーディナリティ列は `text` / `number` フィルターを使います。
