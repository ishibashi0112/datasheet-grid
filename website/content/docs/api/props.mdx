...
| `renderFilterDateInput` | `(ctx: FilterDateInputContext) => ReactNode` | ネイティブ `<input type="date">` | dateSet フィルター条件の日付入力を利用側コンポーネント(Mantine `DatePickerInput` 等)へ差し替えるスロット。値契約と外側クリック対策は[ソートとフィルター](/docs/guides/filter-sort)を参照。 |
