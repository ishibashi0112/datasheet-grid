...
export const useColumnChooserController = ({
  ...
  //   ごとに呼びます)。位置は clamp して layout へ即時反映し、以後 updateColumnChooserLayout は
  //   gridRoot 追従をやめてこの位置を保持します(closeColumnChooser で解除 = 再オープンは既定位置)。
  //   注記: view 側がドラッグ session の closure に閉じ込めて呼ぶため、本関数は参照安定
  //   (deps [])であることが契約です。
  const moveColumnChooser = useCallback((top: number, left: number) => {
    const clamped = clampPanelDragPosition({
      top,
      left,
      panelWidth: PANEL_WIDTH,
    ...
    setColumnChooserLayout({ top: clamped.top, left: clamped.left, width: PANEL_WIDTH });
  }, []);
