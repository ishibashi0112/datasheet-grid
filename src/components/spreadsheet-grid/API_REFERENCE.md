...
- **行番号バブル(`bubble`)**: スクロール中、スクロールバー脇(疑似サム位置)に「行 N / 総行数」を表示。`hintColumn` / `renderHint` で任意の列値を添えられる。
- **行目盛りルーラー(`ruler`)**: ボディ右端に行数スケール(切りのよい刻み。1 万以上は「10万」等へ圧縮表示)。スクロール中とスクロールバー帯ホバー中のみフェード表示。
- **ジャンプ先プレビュー(`ruler` に含む)**: スクロールバー帯(右端)にポインタを置くと、ネイティブスクロールバーのクリック / ドラッグと同じサム中心写像で「行 N へ」の点線 + ラベルを表示。
...
| `trigger` | `'scroll' \| 'hover' \| 'always'` | `'scroll'` | 表示トリガー。`'scroll'` = スクロール中のみ(停止約 1 秒でフェードアウト)/ `'hover'` = グリッドホバー中 + スクロール中 / `'always'` = 常時。 |
...
**実装ノート**: 行番号は仮想化の縦ジオメトリ(1M 行の pixel scaling / auto-height の prefix-sum)と同一の写像で解決するため常に正確。オーバーレイは `pointer-events: none` で、スクロールバー操作・セル操作へ一切干渉しない(ジャンプ先プレビューの帯検知もリスナーのみで実装)。全行が viewport に収まりスクロール不能のときは何も表示しない。
